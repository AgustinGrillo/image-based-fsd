...
def process_img(image):
    ...
    processed_image = image[:, :, 0]  # blue channel
    processed_image = processed_image[190:190 + screen_size[0], :]  # crop: [190:355, :]
...
def smooth_img(img, smooth_type='erosion'):
...
def pixelate_img(img, height_pixelation = 4, width_pixelation = 4):
	height, width = img.shape[:2]

	img_pixelated = cv2.resize(img, (int(width/width_pixelation), int(height/height_pixelation)), interpolation=cv2.INTER_LINEAR)
