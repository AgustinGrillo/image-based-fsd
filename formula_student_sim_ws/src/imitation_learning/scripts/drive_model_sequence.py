...
class cmd_vel_node(object):
    def __init__(self):
        ...
        self.imgRcvd = False
        self.buffer = np.zeros((self.size_buffer, 480, 640, 3)).astype(np.uint8)  # 16
    ...
    def cvt_image(self, data):
        ...
        self.buffer[self.size_buffer] = self.latest_image
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.imgRcvd and self.buffer_full:
                ...
                #                                                                        screen_size)
                preprocessed_image = np.expand_dims(self.buffer[0:self.skip_frames:(self.size_buffer-1)], axis=0)  # we add batch dimension
