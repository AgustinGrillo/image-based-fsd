...
pubR = rospy.Publisher('/fs/controller/position/JointSteerRight/command', Float64, queue_size=1)
pubM = rospy.Publisher('/fs/controller/diff_drive/cmd_vel', Twist, queue_size=1)
...
def callback(data):
    ...
    # angle_beta = math.asin((FH / LA) * math.sin(angle_wheel))

    pubR.publish(angle_wheel)  # instead of angle_wheel should be angle_beta
...
def listener():
    ...
    rospy.Subscriber("/fs/cmd_vel", Twist, callback)  # This topic is published by the joy_node
