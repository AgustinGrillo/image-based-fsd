...
def main():
    ...
    path_imitation_learning = rospack.get_path('fs_hardware_interface')
    output_path = path_imitation_learning + "/output"
    input_path = path_imitation_learning + "/data"

    i = 1
    while os.path.exists(output_path + "/data_track_%s" % i):
        i += 1

    output_folder = output_path + "/data_track_%s" % i
