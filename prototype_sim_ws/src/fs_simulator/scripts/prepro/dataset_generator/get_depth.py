...
if not os.path.exists(PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set'):
    os.mkdir(PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set')
if not os.path.exists(PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set/Target_Depth'):
    os.mkdir(PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set/Target_Depth')
...
for pose_num in range(num_poses):
    ...
    if not os.path.exists(PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set/Target_Depth/mid' + str(pose_num)):
        os.mkdir(PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set/Target_Depth/mid' + str(pose_num))

    list_files = glob.glob(
        PATH_fs_simulator + '/scripts/prepro/dataset_images/training_set/Noise/mid' + str(pose_num) + '/*')
    # if len(list_files) == 0:
    #     texture_iter = 0
    # else:
    #     val = max(list_files, key=os.path.getctime)
    ...
    np.save('../dataset_images/training_set/Target_Depth/mid' + str(pose_num) + '/mid'+str(pose_num) + '_' + str(texture_iter), depth_array_32fc1)
    # Guardamos imagen de referencia. (No deberia ser utilziada para entrenar, dado que tiene precision de 8 bits)
    cv2.imwrite('../dataset_images/training_set/Target_Depth/mid' + str(pose_num) + '/mid'+str(pose_num) + '_reference_' + str(texture_iter) + '.jpg', depth_array*255)
