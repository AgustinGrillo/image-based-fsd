...
# Hyperparameters
TARGET_TYPE = 'RGB'  # 'Simplified' or 'RGB'
IMAGE_SHAPE = (320, 640, 3)  # (192, 640, 3)
IMAGE_VERTICAL_CROPPING = (20, 340)  # (168, 376)
MAX_DEPTH_DISTANCE = 4.0  # Upper rounded (ceil) maximum distance measured by depth camera. This is used for normalization.
RETRAINING_DEPTH = False
RETRAINING_MODEL_PATH = None
DATASET = 'data_set_4'
SAVE_MODEL_FOLDER_NAME = 'model_test_320x640_blank_dataset'
SAVE_FREQUENCY = 2
EPOCHS = 50
BATCH_SIZE = 32
VALIDATION_SPLIT = 1000
SHUFFLE_BUFFER = 25000  # Approx the size of the full dataset.
LAMBDA_GAN = 1
LAMBDA_SL_SIM = 1
LAMBDA_SL_DEPTH = 300
NOISE_PATH_RELATIVE = '/dataset_images/'+DATASET+'/training_set/Noise'

current_file_path = os.path.dirname(os.path.abspath(__file__))
NOISE_PATH = current_file_path + NOISE_PATH_RELATIVE
...
def np_map_func(feature_path, flip):
    ...
    feature_crop = feature[IMAGE_VERTICAL_CROPPING[0]:IMAGE_VERTICAL_CROPPING[1], :]
    if flip:
...
def load_images(imagePath):
    ...
    noise_image = tf.image.crop_to_bounding_box(noise_image, offset_height=IMAGE_VERTICAL_CROPPING[0], offset_width=0, target_height=IMAGE_SHAPE[0], target_width=IMAGE_SHAPE[1])
    ...
    img_num = img_num[-2]
    mid = mid[-2]
    target_path = 'dataset_images/'+DATASET+'/training_set/Target_' + TARGET_TYPE + '/' + mid + '/' + img_num + '.jpg'
    target_image = tf.io.read_file(target_path)
    target_image = tf.image.decode_png(target_image, channels=3)
    target_image = tf.image.crop_to_bounding_box(target_image, offset_height=IMAGE_VERTICAL_CROPPING[0], offset_width=0, target_height=IMAGE_SHAPE[0], target_width=IMAGE_SHAPE[1])
    ...
    # pixels intensities to the range [0, 1]
    depth_path = 'dataset_images/'+DATASET+'/training_set/Target_Depth/' + mid + '/' + img_num + '.npy'
    depth_npy = tf.numpy_function(np_map_func, [depth_path, flip_cond], tf.float32)
    depth_npy = depth_npy/MAX_DEPTH_DISTANCE
    # Normalization: we scale it to [-1, 1]. (Note: that converting to float32 pre-scaled to [0, 1])
...
def train(train_dataset, test_dataset, epochs):
    ...
    save_folder_version = '1'
    save_folder_name = SAVE_MODEL_FOLDER_NAME + '_V' + save_folder_version
    folder_empty = False

    while not folder_empty:
        if not os.path.exists('gan_models/' + save_folder_name ):
            os.mkdir('gan_models/' + save_folder_name)
            folder_empty = True
        else:
            save_folder_version = str(int(save_folder_version) + 1)
            save_folder_name = SAVE_MODEL_FOLDER_NAME + '_V' + save_folder_version
    ...
    for epoch in range(epochs):
        ...
        if epoch % SAVE_FREQUENCY == 0 or epoch == (epochs-1):
            ...
            fig.savefig('gan_models/' + save_folder_name + '/evaluation_validation_epoch_{}'.format(epoch), bbox_inches='tight', dpi=400)

            # fig, axs = plt.subplots(3, 6)
            # plt.subplots_adjust(hspace=0)
            # for idx, val in enumerate(['0002', '0003', '0005', '0007', '0010', '0017']):
            #     orig_img = cv2.imread('/home/agus/Documents/Proyectos/fs_itba/src/fs_simulator/scripts/prepro/dataset_images/real_test_images/prototype/' + val + '.jpg')
            #     orig_img = cv2.cvtColor(orig_img, cv2.COLOR_BGR2RGB)
            #     orig_img = orig_img[20:340, :]
            #     orig_img_plot = (orig_img/255.0)
            #     axs[0, idx].imshow(orig_img_plot)
            #     axs[0, idx].axis('off')
            #     orig_img = 2.0 * orig_img_plot - 1.0
            #     tensor = tf.convert_to_tensor(np.expand_dims(orig_img, 0), dtype='float32')
            #     fake_sim = generator(tensor, training=False)[0][0]
            #     fake_sim = tf.image.convert_image_dtype(0.5*(fake_sim+1.0), dtype=tf.uint8)
            #     fake_sim = fake_sim.numpy()
            #     axs[1, idx].imshow(fake_sim)
            #     axs[1, idx].axis('off')
            #     fake_depth = generator(tensor, training=False)[1][0]
            #     fake_depth = tf.image.convert_image_dtype(0.5*(fake_depth+1.0), dtype=tf.uint8)
            #     fake_depth = fake_depth.numpy()
            #     axs[2, idx].imshow(fake_depth[:, :, 0])
            #     axs[2, idx].axis('off')
            # fig.savefig('gan_models/' + save_folder_name + '/evaluation_test_epoch_{}'.format(epoch), bbox_inches='tight', dpi=400)


        # Saving the model
        if ((epoch % SAVE_FREQUENCY == 0) & (epoch != 0)) or epoch == (epochs-1):
            generator.save('gan_models/' + save_folder_name + '/generator_model_epoch_{}'.format(epoch))
            # tst = tf.keras.models.load_model('gan_models/generator_model_epoch_{}'.format(epoch))
