...
class cmd_vel_node(object):
    def __init__(self):
        ...
        rospy.Subscriber("/fs/bumper", ContactsState, self._contact_callback_)
    ...
    def _contact_callback_(self, data):
        if data.states == [] or data.states[0].collision2_name=='asphalt::link::collision':
            "No collision"
            collision = False
        else:
